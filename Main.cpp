...
void menu(RenderWindow & window) {
	...
	while (isMenu)
	{
		...
		if (IntRect(100, 500, 350, 100).contains(Mouse::getPosition(window))) { menu1.setColor(Color::Blue); menuNum = 1; }// когда наведена мышь
		if (IntRect(100, 600, 350, 100).contains(Mouse::getPosition(window))) { menu2.setColor(Color::Blue); menuNum = 2; }
...
int main() 
{
	...
	RenderWindow window(VideoMode(1024, 768), "Prototip");  //Style::Fullscreen
	menu(window);
	...
	Enemy rat(ratImage, 2400, 2400, 67, 28, "rat");
	...
	while (window.isOpen()) {
		...
		//Update
		//Vectors
		playerCenter = Vector2f(MainHero.getPlayerPositionX(), MainHero.getPlayerPositionY());
